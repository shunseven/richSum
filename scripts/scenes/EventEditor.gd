extends Control
## 随机事件编辑器：图标 emoji + 名称 + 类型 + 描述。

const TYPES := ["reward", "punishment", "assign_task", "npc_system"]
const TYPE_LABELS := {
	"reward": "奖励 ✨",
	"punishment": "惩罚 😤",
	"assign_task": "任务 📝",
...
func _ready() -> void:
	var v := EditorUI.make_screen(self, "❗ 随机事件编辑", func(): SceneRouter.goto(SceneRouter.SCENE_MENU))
	v.add_child(_build_form())
	v.add_child(UIUtil.make_label("现有随机事件", 22, UIUtil.C_GOLD))
...
func _build_form() -> Control:
	...
	pc.add_child(v)
	v.add_child(UIUtil.make_label("新增事件", 22, UIUtil.C_GOLD))
...
func _refresh() -> void:
	for c in list_root.get_children():
		c.queue_free()
	var list: Array = GameStore.get_events()
	for ev in list:
...
func _build_row(ev: Dictionary) -> Control:
	...
	b_save.pressed.connect(func():
		GameStore.update_event(String(ev.get("id", "")), {
	...
	b_del.pressed.connect(func():
		GameStore.delete_event(String(ev.get("id", "")))
		_refresh())
	hb.add_child(b_del)
	return pc
